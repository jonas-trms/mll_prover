...
Notre assistant de preuve a été implémenté en OCaml, et peut être consulté et essayé \href{https://github.com/jonas-trms/mll_prover}{ici}. Nous ferons régulièrement le lien, dans la suite, entre les résultats théoriques que nous présentons, et leur implémentation dans le code.
...
\begin{remark}
    On préserve l'ordre lexicographique sur les feuilles, car on souhaite étiqueter celles-ci par des paires d'adresses. La règle d'axiome étant unique, elle ne demande pas d'ordre sur les atomes, ce qui fait de leur paire une information suffisante. 
    ...
    \begin{example}Deux preuves identiques à permutations près:
        ...
        \begin{multlined}
            ...
            \begin{prooftree}
               ...
               \permv{(2, 1)}{X_1, {X_1}\orth}
            \tensorv{{X_1}\orth, X_1 \tensor X_1, {X_1}\orth}
            \permv{(3, 2, 1)}{{X_1}\orth, X_1 \tensor X_1, {X_1}\orth}
            \end{prooftree}
        \end{multlined}
        \end{align*}
...
\begin{remark}
    ...
    \begin{example}
        ...
        $F(1 \epsilon, \; 3 \epsilon) \vdash X_1, \orth{X_1} \tensor \orth{X_2}, X_2$
...
\begin{itemize}
    ...
    \item Si, pour un trou, seuls des atomes à l'exception d'un unique opérateur obligatoire peuvent être choisis, on applique l'opérateur.
...
\section{Une trace d'exécution de notre assistant}
\label{exec_trace}
    ...
    \begin{description}
        \item[Initialisation:]
            ...
            \begin{description}
                ...
                \item[$\supseteq$]
                Supposons par l'absurde qu'il existe $\beta \in \highapproxspec \left( H, \sequent, \; \epsilon \right)$ telle que $\beta \notin \Sigma$. Une telle adresse serait nécessairement de la forme $(i, \alpha \cdot \someadd)$, et devrait pourtant étiqueter la racine de $t'$, ce qui ne respecterait pas la condition \ref{cdespartial} de descendance.
            \end{description}
