...
\begin{remark}\label{remholecorrect}
...
\begin{algorithmic}
    \Function{approx}{$t, \; a', \; \highapprox, \; \lowapprox$}
        \MatchWith{$t, \; a'$}
            ...
            \Case{$U((m, \someadd), t_1), \; \Left \cdot \someaddbis$}
                ...
                \State \Return $\text{APPROX}(t_1, \; \someaddbis, \; \highapprox, \; \lowapprox)$
            \EndCase
    
            \Case{$B((m, \someadd), t_1, t_2), \; \Left \cdot \someaddbis$}
                \State $\mathcal{M} \gets \left\{a \mid a \text{ est une adresse minimale apparaissant dans $t_2$}\right\}$
...
\begin{proof}
    D'après la condition $\exactcond$ et la spécification de $\highapprox$ (théorème~\ref{highapprox_correction}), on a, par récurrence, $(t_i, \sequent) \in \representationspartial, \; \; \forall 0 \leq i \leq n$.
    En effet, $(t_0=\unknown, \; \sequent) \in \representationspartial$ (remarque~\ref{remholecorrect}) et à chaque fois qu'on pioche une adresse dans $\highapprox$, et qu'on s'en sert pour substituer un trou, la spécification $\highapproxspec$ garantit que l'adresse est correcte. En associant ce résultat à la condition $\exactcond$, on déduit que le bon adressage \caddpartial{} est conservé. Par ailleurs, la spécification $\highapproxspec$ garantit que les adresses utilisées n'apparaissaient pas dans l'arbre qu'on a modifié, et donc que la sous-linéarité \clinpartial{} est conservée. Enfin, toujours d'après la spécification $\highapproxspec$, les adresses utilisées descendent immédiatement d'une adresse racine ou présente sur la branche racine--trou, en respectant les conditions gauche et droite. Cela assure que la descendance \cdespartial{} est conservée. Les représentations partielles sont donc toutes dans $\representationspartial$.
    ...
    \begin{description}
        ...
        $a$ est une adresse racine correcte n'apparaissant pas dans $t$. D'après le théorème~\ref{lowapprox_correction}, $t$ n'ayant qu'un trou adressé par $\beta$, on a donc $a = (m, \epsilon) \in \lowapprox(t_{n-1}, \sequent, \; \beta)$, d'où $\lowapprox(t_{n-1}, \sequent, \; \beta) \nsubseteq \{a_1, \; a_2\}$.
                ...
                \item[$\subseteq$] 
                    ...
                    \begin{description}
                        \item[Si $m = n$:] 
                            ...
                            L'adresse $(n, \rho \cdot \Left)$ ne peut enfin pas apparaître dans $s(t, \alpha_{i+1})$. En effet, elle n'apparaît pas dans $t''$ à cause de la condition de descendance \ref{cdes}, et elle n'apparaît pas non plus dans le reste de l'arbre car elle n'apparaissait déjà pas dans $s(t, \alpha_{i})$, par HR (car $(n, \rho)$ n'y apparaissait pas).
