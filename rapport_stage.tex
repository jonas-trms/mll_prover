...
\begin{remark}
    ...
    \begin{example}Deux preuves partielles identiques à permutations près:
        \begin{equation*}
            \begin{prooftree}
            ...
            \end{prooftree}
             \begin{prooftree}
                \hypv{X_2, {X_4}\orth, X_3, {X_1}\orth}
               \permv{(4, 1, 3, 2)}{{X_1}\orth, X_2, X_3, {X_4}\orth}
            \parrv{{X_1}\orth, X_2 \parr X_3, {X_4}\orth}
            \end{prooftree}
        \end{equation*}
    \end{example}
...
\begin{proof}
    ...
    \begin{description}
        \item[$F:$] 
            ...
            De la même manière que dans la preuve du théorème~\ref{rep_inversion}, on a $t = F(1 \epsilon, \; 2 \epsilon)$, et $\sequent = X\orth, X$ ou $\sequent = X, X\orth$, avec $X$ un atome. Dans les deux cas, la suite $H, \; F(1 \epsilon, \; 2 \epsilon)$ convient. En effet, celle-ci est une suite d'approximations immédiates, et les deux conditions d'exactitude par rapport à $\sequent$ sont vérifiées:
