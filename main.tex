...
\begin{demonstration}
    ...
    \begin{itemize}
        ...
        $A \parr B$, quant à lui, est étiquetté par la racine, et c'est le seul endroit où son adresse apparaît, car, par construction de $\psi_\parr$, aucun noeud de $\psi_\parr \left( \encode' \left( \pi_1 \right) \right)$ ne peut être étiquetté par $n \epsilon$.

        Finalement, chaque sous-adresse de $\Gamma, A \parr B, \Delta$ apparaît bien une unique fois dans l'arbre, d'où la linéarité est préservée.
        \item[(iii) :]
    \end{itemize}
